...
Main:
	...
	; i will try to get the robot to stop and turn if it something is within one foot?
	;10/31/19 I started adding code here
	;I also want to enable sonar interrupts within the checkfront code?
	LOAD FMid 
    STORE DVel ;gets the robot moving forward
	LOADI 0;
	STORE DTheta ; make sure the robot just moves forward
	SEI  &B0001
CheckFront:
    LOAD   &B11111111 ;I will enable all the sonars
    OUT    SONAREN ; this will enable sonars 0 through 5
	                    ;Use sonar interrupts
						;The value written to IO location SONALARM is used as an alarm
						;distance by the sonar module which can then be read from the same sonalarm
						;address 
						;sonarint does not affect the alarm register sonar alarm
						;but SONAREN 00100010 will only let sonars 5 and 1 cause interupts
						;the value returned from SONALARM is the 8 bit addres vector
						;corresponding to the sonar alarm that was within the alarm distance
						;if this doesnt work then try using sonar interupts
						;i did not use an interrupt i just used a jump branch 
	; this enables sonar interupts?
    LOAD Ft1.5 ;load the mm value for one foot into the accumulator
    OUT SONALARM
CheckDist: ;sets the alarm distance to one foot
    ...
    JUMP  CheckDist ;if not keep checking front
StopAndTurn:
    ;diable the sonar interrupts
   
	
   ; STORE  AlarmTrigger ;save the sonar that triggered the alarmm ;does the jump instruction change the accumulator
	CLI &B0001
	LOAD  Zero  ;stop the movement API!
	STORE  DVel
	LOAD AlarmTrigger
	OUT SSEG1 ;out the alarm trigger to the seven segment display
    ;write a series of and statements that will check which sonar that the alarm triggered from
	;and face toward it!
	LOAD  AlarmTrigger
	AND  Mask0
	JPOS Stop&ShowTheta ;no need to turn if already normal to it 
	LOAD AlarmTrigger
	AND  Mask1   ;if the robot triggers at sensor 1 then turn to the right 46 degrees to make it normal to sensor 0
	JPOS Turn46Right
	LOAD AlarmTrigger
	AND  Mask2   ;if the robot triggers at sensor 2 then turn right 78 degrees to make it normal to sensor 0
	JPOS Turn78Right
	LOAD AlarmTrigger
	AND  Mask3
	JPOS Turn78Left  ;if the robot triggers sensor 3 then turn left 78 degrees 
	LOAD AlarmTrigger
	AND  Mask4  ; if sensor 4 is triggered then turn robot left 46 degrees 90-44
	JPOS Turn46Left
	LOAD AlarmTrigger
	AND  Mask5
	JPOS Stop&ShowTheta ;but make it a box or a circle
	
	LOAD AlarmTrigger
	AND  Mask6
	JPOS Turn54Right ;change this 
	
	LOAD AlarmTrigger
	AND  Mask7
	JPOS Turn54Left ; change this as well 
...
Turn46Left:
	...
	STORE DTheta
	STORE DesiredTheta
	JUMP  CheckTheta ;instead of jumping to die i need to check if Dtheta abd theta have matched 
Turn78Left:
    LOAD Deg78
	STORE DTheta
	STORE DesiredTheta
...
CheckTheta:
    ...
    ; write a subroutine to make sure that the robot has turned the correct amount of degrees
;	IN THETA
;	SUB DesiredTheta
;	CALL Abs
;	ADDI  -3
...
;TurnLoop:
	...
	; at this point, robot should be within 3 degrees of 90
                  ...
                  ; use API to move forward
	              ;lets read from the forward sensors
...
DataArray:
	DW 0
;Thomas Talbot I came down here and added
;some variables
AlarmTrigger: DW 0
Deg46:    DW 46
Deg78:    DW 78
