...
StartMoving:
	 ...
	 SEI &B0001
	 LOADI &B00000101 ;only enable sonar 0, and 2
     OUT SONAREN  
...
WaitForSonar0:
     ;this waits for sonar 0 to trigger in order to start moving 
...
ReadSonars:
    IN SONALARM
    AND Mask2 ; if sensor two is positive i am not safe i need to turn to the right!
    JPOS TurnRight
...
ShouldITurn:
	...
	IN SONALARM 
	AND Mask0 ;if sonar 6 is triggering then keep going straight
...
TurnLeft:
    ...
    OUT RESETPOS
    LOADI  20
    ;ADDI 45 ;load the current theta 
	 ; add 45 degrees
	STORE DTheta 
	JUMP CheckTheta2;
	;LOAD TurnCount
	...
	;LOAD TurnCount
	;OUT SSEG2
	;JUMP CheckTheta2 ; maybe jump to check theta 
...
TurnCount: DW 0	
StaticTurnCount: DW 0 

	 
	 
	 
CheckTheta: ;this check theta subroutine jumps back to the beginning code 
     CALL GetThetaErr
     CALL Abs
	 ADDI -3
	 JPOS CheckTheta
