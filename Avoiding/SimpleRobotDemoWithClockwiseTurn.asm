...
Main:
	...
	;LOADI  90 ;Thomas Commented this out 10/30
	;STORE  DTheta      ; use API to get robot to face 90 degrees

;Below I have added the subroutine to turn clockwise until sensor 5 reads something within 5 feet
;my first action should be reading sensor 5
;
;I believe the timer interupts are already happening in the main code
ResetOrientation:  OUT RESETPOS ;resets odometry to (0,0,0)
                   LOAD  Deg270  ;I believe 270 will turn robot to the right if it doesnt then try -90
		           STORE DTheta  ;set the desired angle
CheckTheta:

          
        IN    THETA  ;this will check how the robot is oriented
		ADDI  -270 ;find the difference between actual and desired heading
		CALL   Abs 
		ADDI   -3 
		JNEG   ResetOrientation ; if the error is less than three degrees than
		                        ;reset the orientation and keep turning!
        LOAD   Mask5  ;loads 5 into the accumulator
		OUT    SONAREN ;enables sonar 5
		IN     DIST5 ;reads sonar 5 which is in mm 
		ADDI   -1220  ; 
		JNEG   Die   ; if the distance between sonar 5 and some other object  is less 
		             ;then 4ft than call subroutine die
		JUMP   CheckTheta  ;else keep checking theta
