...
StartMovingForward:
	...
	SEI  &B0001 ;enable sonar interrupts
	LOAD   &B00011110 ;I will enable sonars 1 through 4 only! 
    OUT    SONAREN
	LOAD ThreeAndAHalf ;set the alarm distance to 3 and a half feet
	OUT SONALARM

ReadAlarm:
                ;sets the alarm distance to one foot
...
StopAndTurn:
   ...
   ; STORE  AlarmTrigger ;save the sonar that triggered the alarmm ;does the jump instruction change the accumulator
	...
	LOAD  AlarmTrigger
	AND  Mask1
	JPOS Turn44Left ;turn the robot toward whatever triggered it 
	LOAD AlarmTrigger
	AND  Mask2   
	JPOS Turn12Left
	LOAD AlarmTrigger
...
CheckTheta:
	...
	ADDI -3
	JPOS CheckTheta
	JUMP MoveForwardTowardRelector

   

	
MoveForwardTowardReflector:
  ;only after the robot has turned toward whatever triggered it then we can move 
  ; forward again
  OUT RESETPOS ; reset the alignment of the robot so that theta is zero again 
  LOAD FSlow
  STORE DVel
  LOAD Zero
  STORE DTheta ;move the robot ahead slow 
  ;lets set a new alarm trigger distance to be 1 and a half feet 
  LOAD Ft1AndAHalf
  OUT SONALARM ;this sets the new sonar alarm distance to be a1 and a half feet 
...
ReadAlarmAndStop:
	...
	STORE AlarmTrigger
	LOAD AlarmTrigger
	JPOS StopAndShowAlarm  ; if a bit has been set to positive then jump to another subroutine
	     ;The sonar that it came from is in the accumulator 
    JUMP  ReadAlarmAndStop

StopAndShowAlarm:
   LOAD Zero
   STORE DVel ;stop the robot from moving 
   LOAD AlarmTrigger 
   OUT SSEG1  ;show which sonar triggered the alarm 
   ;if we can get this function working then from here we could 
   ; jump to a circling function 
...
ThreeAndAHalf: DW 1025 ;this is the greatest number that can be represented by the addi 
DataArray:
	DW 0
;***************************************************************
