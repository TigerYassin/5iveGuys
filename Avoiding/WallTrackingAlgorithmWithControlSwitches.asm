...
LookForRight:
	...
	JUMP LookForRight
;MAKE A NEW SUBROUTINE To in dist0 
;if less than 400 then turn 1 degree to the right 
; if greater than 400 then turn on degree  to the left
; if in between do nothing 

WallTracker:
    IN DIST0 
    ADDI -350
    JNEG Turn10DegreeRight ;if less than 350
    IN DIST0 
    ADDI -500
    JPOS Turn10DegreeLeft ;if greater than 500 
    JUMP LookForRight 
...
CircleCompleted:
   ...
   ; LOAD CircleCount
    ...
    ;LOAD CircleCount
    ;ADDI -3
    ;JPOS Start ;if JPOS then i will just continue circling I NEED TO FIGURE OUT WHAT THIS DOES I changed this on 11/11
;----HERE IS WHERE I WILL ADD IN CODE FOR THE PLAN B SLIDE SWITCHES
    ;IN SWITCHES
    ;AND Mask0 ; if switch 0 is high then keep circling 
    ;JPOS Start ;see if any of the slide switches our active then keep circling	
	;OUT RESETPOS
...
;------	IN BETWEEN HERE AND THE NEXT ALL CAPS COMMIT THESE SUBROUTINES ARE NOT USED
GoBackToWall:
    LOAD Fslow
	STORE DVel
...
TurnRight90:
	...
	CALL Wait1
	CALL Wait1
	CALL Wait1 ;let the robot get away from the reflector so that it can find a new one 
...
CheckTheta3:
     ...
     ADDI -3
     JPOS CheckTheta3
     JUMP StartMovingForward ;instead of going back to wall
