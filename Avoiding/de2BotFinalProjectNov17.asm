...
StartMovingForward:
    ...
    STORE DTheta
    LOADI  250
    STORE DVel			; move forward medium 
	
    ; enable sonars 2&3 for wall detection, sonar 5 for reflector detection
	LOADI &B00001110
	OUT   SONAREN
	LOADI  400 	;set the alarm distance to .4m
	OUT   SONALARM
...
SetAlarmDistance:	
	...
	;AND Mask0 
	JUMP WallTracker
;The wall tracking subroutine checks if the distance between the robot and the wall is within a certain range
;iif it is too close or too far from the wall it will adjust this range in incrememnts of 3 degrees
ShortDistance:
    IN DIST5
    ADDI -1000
    JNEG RotateToRefl
    JUMP SetAlarmDistance
LongDistance:
    IN DIST5
    ADDI -1250
    JNEG  RotateToRefl
...
WallTracker:
    IN DIST0 
    ADDI -425
    JNEG Turn10DegreeRight ;if less than -425
   ;IN DIST0 
    ;ADDI -475
    ;JPOS Turn10DegreeLeft ;if greater than 475 
    JUMP LookForRight 
...
CheckTheta5:
     ...
     ADDI -3
     JPOS CheckTheta5
     JUMP LookForRight ;is this correct yes it is 
...
RotateToRefl:
	...
	OUT		SSEG1
	LOADI 	&B0011110 		; would be used to enable just the front sonars
	OUT		SONAREN
...
ForwardToRefl:
	...
	LOADI	400				; reduce distance from bot to reflector
	OUT		SONALARM
	IN  	SONALARM 
	;JNEG	ForwardtoRefl
	JPOS 	TurnToBeginTurning			; Bot has reached 0.4m out from a reflector
	JUMP 	ForwardToRefl
...
StartMoving:
     ...
     OUT SONAREN  
     LOADI 400 ;load the alarm distance
	 OUT SONALARM
...
TurnLeft:
	...
	LOAD TurnCount 
	ADDI -19 ;changed to make the number of turns less 
...
TurnRight:
    LOAD TurnCount
	ADDI -1
	STORE TurnCount ;subtract 1 from turn count 
    OUT RESETPOS
	LOADI -20
	STORE DTheta
	JUMP CheckTheta2
...
CircleCompleted:
	...
	JUMP CheckCircleCount

;--------HERE IS THE CONTROL SWITCH CODE ADDED ON 11/13 THIS SETS THE NUMBER OF TIMES THE ROBOT CAN TURN

;----I will add a seperate subroutine that will adjust the number of times we circle 
;----based of slide switch 2 3 4 
;----slide switch 2 allows 2 turns
;-----slide switch 3 allows 3 revolutions
;-----slide switch 4 allows 4 revolutions
;-----if none our present then just try to do loops forever
CheckCircleCount:
    LOAD CircleCount
	ADDI 1
	STORE CircleCount
	;increment ciircle count by 1
	IN SWITCHES 
	AND Mask0
	JPOS Start ;if slide swithc 0 up then just keep circling
	IN SWITCHES 
	AND Mask2
	JPOS TwoRevolutions
	IN SWITCHES
	AND Mask3
	JPOS ThreeRevolutions
	IN  SWITCHES
	AND Mask4
	JPOS FourRevolutions
	OUT RESETPOS
	LOADI -90 ;turn 90 degrees so i can enter my exploring subroutine 
	STORE DTheta ;if this code works then the robot will 
	JUMP CheckTheta3 ; go back to wall subroutine  will exist in thew explore and circle.asm 

TwoRevolutions:
    LOAD CircleCount ; we need to check if circle count is 2 or greater
	ADDI -1
	JPOS Start
	OUT RESETPOS
	LOADI -90 ;turn 90 degrees so i can enter my exploring subroutine 
...
;--------ABOVE HERE IS THE CONTROL SWITCH CODE THAT TELLS THE ROBOT HOW MANY TIMES THE ROBOT CAN TURN


;------	IN BETWEEN HERE AND THE NEXT ALL CAPS COMMIT THESE SUBROUTINES ARE NOW USED 11/13
GoBackToWall:
    LOADI 200 
	STORE DVel
	LOADI 500 ;just jump to the infinite loop for now 
...
CheckFront:
    ...
    IN SONALARM
	AND Mask1 
	JPOS TurnRight90 ;change these to be different angles instead of 90 
	IN SONALARM
	AND Mask2
	JPOS TurnRight90
	IN SONALARM
...
TurnRight90:
	...
	LOADI -90 
	STORE DTheta
	; away from the reflector so that it can find a new one 
	JUMP CheckTheta6 ;----HERE IS WHERE IT JUMPS TO THE EXPLORING CODE 
TurnRight135:
    OUT RESETPOS
    LOADI -135
	STORE DTheta
...
;-------THE CODE ABOVE WAS IMPLEMENTED ON NOV 13 BY THOMAS TALBOT
...
CheckTheta3:
     ...
     ADDI -3
     JPOS CheckTheta3
     JUMP GoBackToWall ;instead of going back to wall
CheckTheta6:
     CALL GetThetaErr
     CALL Abs ;only care about absolute error
     ADDI -3
     JPOS CheckTheta6
     JUMP LookForRight ;instead of going back to wall
